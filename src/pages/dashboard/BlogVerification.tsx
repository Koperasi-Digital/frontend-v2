...
export default function BlogVerification() {
  ...
  const { blogVerificationList, refresh } = useSelector((state: { blog: BlogState }) => state.blog);
  ...
  const handleVerifyBlog = async (blogId: number) => {
    try {
      await dispatch(setVerified(blogId));
      enqueueSnackbar(`Verifikasi blog berhasil diubah`, { variant: 'success' });
    } catch (err) {
      console.error(err);
      enqueueSnackbar(`Gagal mengubah verifikasi blog, mohon dicoba lagi!`, { variant: 'error' });
    }
  };
  ...
  return (
    <Page title="Blog Verification | CoopChick">
      <Container maxWidth={false}>
        ...
        <Card>
          <BlogVerificationToolbar filterTitle={filterTitle} onFilterTitle={handleFilterByTitle} />
