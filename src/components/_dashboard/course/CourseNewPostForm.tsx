...
export default function CourseNewPostForm() {
  ...
  const formik = useFormik<NewPostFormValues>({
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      try {
        ...
        await dispatch(createCourse(user?.id, values.title, values.description, uploadFileMessage));
        resetForm();
        setSubmitting(false);
        enqueueSnackbar('Create Course success', { variant: 'success' });
      } catch (error) {
        console.error(error);
        setSubmitting(false);
        enqueueSnackbar('Create Course failed', { variant: 'error' });
      }
    }
  });
